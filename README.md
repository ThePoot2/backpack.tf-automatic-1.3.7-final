...
Fixed all known bugs that are currently known for Backpack.tf automatic.
(2019/6/3)

Including:

- Australium weapons are now reliable to trade again. Whenever it gets confused it will retry or decline.
- Unusual effects are now properly recognized.
- Fixed rare bug where killstreak weapons don't have any killstreak info attached. Will retry if this happens.
- Metal to key conversion is still disabled since market changes too much to track.
- Optimized the mobile authenticator loop so it will accept faster in some cases.

- + all fixes from backpack.tf-automatic-1.3.6
